...
aller_BOUCLE2
    ...
    goto BOUCLE2
    goto aller_BOUCLE2
; Routine de délai de 160 ms environ
Delay160ms:
    MOVLW   d'250'         ; Chargez la valeur de la boucle externe
    MOVWF   cpt1
DelayOuterLoop:
    MOVLW   d'800'         ; Doublez la valeur de la boucle interne pour un délai plus long
    MOVWF   cpt2
DelayInnerLoop:
    DECFSZ  cpt2, F        ; Décrémentez et sautez si zéro
    GOTO    DelayInnerLoop ; Répétez la boucle interne
    DECFSZ  cpt1, F        ; Décrémentez la boucle externe et sautez si zéro
    GOTO    DelayOuterLoop ; Répétez la boucle externe
    RETURN

Animation1: 
    ;// 2 coup d'horloge
    CALL all_green_1
    CALL Delay160ms  ; Appel de la routine de délai
    CALL color_off
    CALL Delay160ms  ; Appel de la routine de délai   
...
Config_Button:
    BANKSEL TRISB
    BSF TRISB, 1 ; Configurer RB0 comme entrée pour le bouton
    BSF TRISB, 2
    
    ; Répéter pour d'autres boutons si nécessaire
    RETURN
